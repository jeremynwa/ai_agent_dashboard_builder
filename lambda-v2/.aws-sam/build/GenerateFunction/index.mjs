...
var USE_BETA_API = process.env.USE_BETA_API === "true";
var DASHBOARD_SKILL_ID = process.env.DASHBOARD_SKILL_ID || null;
var DATA_ANALYZER_SKILL_ID = process.env.DATA_ANALYZER_SKILL_ID || null;
var INDUSTRY_SKILL_IDS = {
...
async function callClaude({ system, messages, skills = [], maxTokens = 16384, model = "claude-sonnet-4-20250514", temperature = 0 }) {
  ...
  const response = await anthropic.beta.messages.create({
    ...
    container: {
      skills: skills.map(
        (s) => typeof s === "string" ? { type: "custom", skill_id: s, version: "latest" } : s
      )
    },
...
var handler = async (event) => {
  ...
  try {
    ...
    if (screenshot && prompt === "__VISION_ANALYZE__") {
      let codeContext = "";
      if (existingApp) {
        for (const [p, c] of Object.entries(existingApp)) codeContext += `--- ${p} ---
${c}
