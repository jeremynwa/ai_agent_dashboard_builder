...
function fixJsxCode(code) {
  let f2 = code;
  f2 = f2.replace(/\|([^|]+)\|\s*>\s*(\d)/g, '{"|\u200B$1\u200B| > $2"}');
  f2 = f2.replace(/\|([^|]+)\|\s*<\s*(\d)/g, '{"|\u200B$1\u200B| < $2"}');
  f2 = f2.replace(/([A-Z])\s*>\s*(\d)/g, '{"$1 > $2"}');
  f2 = f2.replace(/([A-Z])\s*<\s*(\d)/g, '{"$1 < $2"}');
  f2 = f2.replace(/\u2191/g, "(hausse)");
  f2 = f2.replace(/\u2193/g, "(baisse)");
  f2 = f2.replace(/\u2192/g, "implique");
  return f2;
}
var SYSTEM_PROMPT = `Tu es un expert React senior. Tu generes des applications d'analyse PROFESSIONNELLES style App Factory.
...
async function loadRules() {
  ...
  try {
    ...
    console.error("Rules load error:", e2);
  }
  return rules;
}
